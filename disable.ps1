##################################################
# HelloID-Conn-Prov-Target-OzoVerbindzorg-Disable
# PowerShell V2
##################################################
...
try {
    ...
    if ($null -ne $correlatedAccount) {
        ...
        $dryRunMessage = "Disable OzoVerbindzorg account: [$($actionContext.References.Account)] for person: [$($personContext.Person.DisplayName)] will be executed during enforcement"
    } else {
        $action = 'NotFound'
    ...
    if (-not($actionContext.DryRun -eq $true)) {
        switch ($action) {
            'DisableAccount' {
                Write-Information "Disabling OzoVerbindzorg account with accountReference: [$($actionContext.References.Account)]"
                $splatParams = @{
                    Uri         = "$($actionContext.Configuration.BaseUrl)/scim/v2/Users/$($actionContext.References.Account)"
                    Method      = 'PATCH'
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditMessage = "Could not disable OzoVerbindzorg account. Error: $($errorObj.FriendlyMessage)"
        Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    } else {
        $auditMessage = "Could not disable OzoVerbindzorg account. Error: $($_.Exception.Message)"
